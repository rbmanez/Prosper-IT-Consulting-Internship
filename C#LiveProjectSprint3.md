...
The `Edit` view was throwing an error because in the `ShiftTimes` controller's `Edit` GET method, its `id` parameter was assigned the wrong data type of `Guid`. On the other hand, the `ShiftTime` model (used in the `Edit` view) assigns its `id` property with the data type of `int` data type.
